// ⚠️ EMPLACEMENT CRITIQUE : ce fichier DOIT être dans src/ (le projet utilise
// un dossier src/) — Next.js ignore SILENCIEUSEMENT un instrumentation.ts
// placé à la racine dans ce cas. Il a vécu à la racine pendant des mois :
// register() ne tournait jamais (ni Sentry serveur, ni pre-warm, ni recovery
// au boot, ni scheduler canary, ni worker pg-boss). Vérifier après tout
// déplacement : /api/health doit rapporter queueWorker:"ok".
export async function register() {
  if (process.env.NEXT_RUNTIME === "nodejs") {
    ...
    import("./services/processing/job-recovery")
    ...
    import("./services/processing/queue")
    ...
    import("./services/monitoring/canary")
